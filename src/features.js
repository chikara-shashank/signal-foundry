...
export function aggregate(bars, minutes) {
  ...
  return [...groups.entries()].filter(([start, bs]) => bs.length === minutes && bs[0].ts === start && bs.at(-1).ts === start + width - 60000)
...
export class Features {
  history = new Map();
  add(b, now) {
    if (!validBar(b) || b.ts + 60000 > now + 1000) return null;
    let bs = this.history.get(b.symbol) ?? [];
    if (bs.length && b.ts <= bs.at(-1).ts) return null;
    if (bs.length && b.ts - bs.at(-1).ts !== 60000) bs = [];
    ...
    const f5 = aggregate(bs, 5), f15 = aggregate(bs, 15);
    ...
    return {
      symbol: b.symbol, version: b.ts, bar: b, count: bs.length, previous: p,
      ...
      trend5: f5.length >= 2 ? f5.at(-1).close / f5.at(-2).close - 1 : null,
      trend15: f15.length >= 2 ? f15.at(-1).close / f15.at(-2).close - 1 : null,
    };
  }
}
